    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link dropdown-toggle" href="#" id="messageDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          ...
          <div id="conteo_solicitudes_burbuja">
        ...
        <div class="dropdown-menu p-0" aria-labelledby="messageDropdown">
          ...
          <div class="p-1" id="detalle_solicitudes">
            <a href="{{ url('/gestion_solicitudes/solicitud/1/leer') }}" class="dropdown-item d-flex align-items-center py-2">
              <div class="me-3">
                <img class="wd-30 ht-30 rounded-circle" src="{{ url('https://portal.unag.edu.hn/matricula/documentos/fotos/1756945640.JPG') }}" alt="userr">
              </div>
          ...
          <div class="px-3 py-2 d-flex align-items-center justify-content-center border-top">
            <a href="{{ url('/gestion_solicitudes/recibidas') }}">Ver todo</a>
          </div>
        </div>
      </li>
      @endif
      ...
      <li class="nav-item dropdown">
        <a class="nav-link dropdown-toggle" href="#" id="profileDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <img class="wd-30 ht-30 rounded-circle" src="https://portal.unag.edu.hn/matricula/documentos/fotos/{{@session('foto')}}?v={{time()}}" alt="profile" onerror="this.onerror=null; this.src='{{ url(asset('/assets/images/user2-403d6e88.png')) }}';">
        </a>
        <div class="dropdown-menu p-0" aria-labelledby="profileDropdown">
          <div class="d-flex flex-column align-items-center border-bottom px-5 py-3">
            <div class="mb-3">
              <img class="wd-60 ht-60 rounded-circle" src="https://portal.unag.edu.hn/matricula/documentos/fotos/{{@session('foto')}}?v={{time()}}" alt="" onerror="this.onerror=null; this.src='{{ url(asset('/assets/images/user2-403d6e88.png')) }}';">
            </div>
...
function alertaSolicitudes() {
  $.ajax({
    url: "{{ url('/alerta_solicitudes') }}",
    method: 'GET',
    success: function(solicitudes_nuevas) {
      //console.log(solicitudes_nuevas.length)
      $("#conteo_solicitudes_burbuja").html(`<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger text-white">
                `+solicitudes_nuevas.length+`
                <span class="visually-hidden">unread messages</span>`);
      $("#conteo_solicitudes").html(solicitudes_nuevas.length+` solicitudes nuevas`);
      for(var i = 0; i < solicitudes_nuevas.length; i++){
        ...
        $("#detalle_solicitudes").append(`<a href="{{ url('/gestion_solicitudes/solicitud/`+row.id_solicitud+`/leer') }}" class="dropdown-item d-flex align-items-center py-2">
              ...
              <div class="d-flex justify-content-between flex-grow-1">
                <div class="me-4">
                  <p>`+row.remitente+`</p>
                  <p class="tx-12 text-muted">`+row.descripcion+`</p>
                </div>
                <p class="tx-12 text-muted">`+row.fecha+`</p>
              </div>	
            </a>`);
      }
    },
    error: function() {
      $('#sidebar-menu').html('<p>Error cargando datos</p>');
    }
  });
}
